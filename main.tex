...
In particular, when immunity provides protection against symptoms, but not against infections or deaths, epidemic trajectories can have faster infection growth rates and higher peak, which in turn can lead to more deaths.
Nonetheless, even a modest protection against infection can mitigate the population-level effects of asymptomatic spread .

\section*{Significance}

Asymptomatically infected individuals may avoid severe outcomes but can still transmit to others, who can then develop symptoms and experience severe outcomes.
Here, we explore the population-level effects of asymptomatic spread in the presence of strong transmission reduction among symptomatic individuals from behavioral and intervention efforts.
In this case, the number of infections increases with asymptomatic prevalence, whereas epidemic fatalities peak at intermediate levels of asymptomatic prevalence.
We integrate our findings to understanding the effects of interplay between behavior and immunity on disease spread and shed light on the dynamics of future variants.
...
In doing so, we assume that symptomatic individuals can reduce their transmission, reflecting changes in behavior (e.g., self-isolation after symptom onset) or nonpharmaceutical intervention measures.
Under this assumption, we show that the prevalence of asymptomatic infections could paradoxically make population-level outcomes worse than if SARS-CoV-2 was more dangerous at the individual level.
We further extend our framework to understand the interaction between immunity against symptomatic infections on the dynamics of emerging variants.
...
We then evaluate the effects on population-level mortality of changing the asymptomatic proportion $p$ while holding the fatality rate for \emph{symptomatic} cases, $f$, constant (the IFR $(1-p)f$ thus decreases as $p$ increases).
\begin{figure}[!ht]
