...
-- when $\RR_a = 0.25 \RR_s$ and $\delta$ is large, $\RR_0 < 1$ and the epidemic does not take off.
...
In this case, we find that intermediate values of asymptomaticity lead to the highest epidemic fatalities at high $\delta$ values across all ranges of $\RR_a/\RR_s$ we consider.
...
To do so, we reparameterize the model based on relative importance of \textit{non-symptomatic} transmission, rather than on the proportion of of asymptomatic cases.
...
While 60\% reduction in transmission after symptom onset ($\delta_s = 0.6$) is still high, it is likely \jd{in fact, tautologically} more feasible than reaching $\delta = 0.6$.
\jd{I think we can just say something like “it does not seem unrealistic”.}
...
Based on early estimates for the contact tracing effectiveness \citep{kucharski2020effectiveness} we believe that high levels are $\delta_s$ are plausible, but a great deal of uncertainty remains. 
...
Our work further illustrates the potential for asymptomatic infection and transmission to make population-level outcomes worse, but remains primarily theoretical.
Applying these ideas to real outbreak scenarios will require narrowing down uncertainties in key parameters, such as the degree of symptom-responsive transmission reduction and immunity profiles.
...
We also showed that asymptomatic infections can have important implications for evolutionary dynamics, but their contributions in driving \jd{What?: vaccination and} evolutionary dynamics of SARS-CoV-2 is yet unclear.
