...
\begin{flushleft}{
	\Large
	\textbf\newline{
		Intermediate levels of asymptomatic transmission can lead to highest levels of epidemic fatalities
	}
}
\newline
\\
...
\jd{This is an awkward place to cite ourselves (we do it again later) and it makes the sentence a bit awkward, too. Can we find a mainstream cite and say something more like “total number of infections”?}
...
\jd{Do we need this s.?} Early response work estimated that 17.9\% (95\% CI: 15.5\%--20.2\%) of confirmed infections on the Diamond Princess Cruise Ship were asymptomatic; but such estimates neglected undetected cases \citep{mizumoto2020estimating}.
...
however, differences between inferred total viral load from Ct values and infectious viral load add uncertainties to how well asymptomatic individuals can transmit relative to that of symptomatic individuals \jd{Is there a citation? Can this be clarified? Anything about the role of symptoms (coughing, sneezing) in potentially spreading disease?}.
...
\jd{If this is about the possibility of \emph{transmission} via long COVID, we need to say so} asymptomatic SARS-CoV-2 infections present additional challenges due to the possibility of long COVID \citep{xie2022long}.
...
We simulate the model for 365 days, assuming $\beta_s = 0.8/\mathrm{day}$, $\beta_a = 0.75 \beta_s$, $\mu=0.5/\mathrm{day}$, $\gamma_s=\gamma_a=0.2/\mathrm{day}$, and $f=0.01$, and an initial exposed proportion of $10^{-4}$.
\jd{Can we change $\mu$ to $\nu$ to follow convention? Don't feel strongly about $\nu$ but $\mu$ has an accepted meaning here.}
}
\label{fig:base}
\end{figure}
...
\jd{The ff.~is not quite correct: we need $\delta > 1-/\RR_s$ for $p_c>0$ (and we're also assuming $\RR_a>1$.}
...
In contrast, when $\delta$ is small enough such that $\RR_s\geq\RR_a$, \jd{Note that this is the same as the $\delta > 0.25 $ criterion above} then total fatalities decrease with $p$ because because both the number of infections and the IFR ($(1-p)f$) decrease with increasing $p$.
...
One exception is the extreme (and unrealistic) case, in which all non-symptomatic transmission is caused by pre-symptomatic transmission;
in this case, total infections and fatalities are maximized when all transmission is caused by pre-symptomatic transmission. \jd{What?? If all transmission is pre-symptomatic, then we get no fatalities in our model.}
...
\jd{This is continuing to seem tangled to me. Should we reconsider the approach?}
