...
simulate_base_sens_fixR0 <- rdsRead()

simulate_base_max <- simulate_base_sens_fixR0 %>%
  group_by(sim) %>%
  filter(
    time==max(time)
  ) %>%
  mutate(
    beta_a_ratio=factor(beta_a_ratio, 
...
g1 <- ggplot(simulate_base_max) +
  ...
  facet_wrap(~beta_a_ratio, labeller = label_parsed) +
  theme(
