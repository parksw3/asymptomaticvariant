...
Moreover, at least half of the infections are sufficiently mild that they could be classified as subclinical\DIFdelbegin \DIFdel{or even asymptomatic}\DIFdelend . 
...
\DIFaddbegin \DIFadd{It is now well established that asymptomatic individuals can transmit SARS-CoV-2 infections \mbox{%DIFAUXCMD
\citep{gao2021role,johansson2021sars,subramanian2021quantifying,koelle2022changing,lizewski2022navy}}\hspace{0pt}%DIFAUXCMD
, but the majority of asymptomatic cases are now driven by prior immunity.
...
\DIFaddbegin \DIFadd{Notwithstanding similarities in the viral load dynamics, the presence (or the absence) of symptoms further contributes to the uncertainty in the differences in transmissibility of symptomatic and asymptomatic individuals.
In general, coughing and sneezing can help deliver virus containing droplets, the prevalence of presymptomatic and asymptomatic transmission of SARS-CoV-2 suggests speech droplets can also be an important mode of transmission \mbox{%DIFAUXCMD
\citep{stadnytskyi2021breathing}}\hspace{0pt}%DIFAUXCMD
.
}

\DIFaddend We note also that asymptomaticity can change across outbreak settings \cite{alene2021magnitude}.
...
\DIFaddbegin \DIFadd{We note that these results are robust to uncertainties in asymptomatic transmission rate.
...
\DIFdelbegin \DIFdel{We therefore }\DIFdelend \DIFaddbegin \DIFadd{To further address the uncertainty in the values of $\delta$, we }\DIFaddend extend our model to \DIFdelbegin \DIFdel{consider the effects of generalized }\textit{\DIFdel{non-symptomatic}} %DIFAUXCMD
\DIFdel{transmission, which includes }\DIFdelend \DIFaddbegin \DIFadd{include }\DIFaddend both pre-symptomatic and asymptomatic transmission.
\DIFdelbegin \DIFdel{For this model, we assume that $\delta$ decreases transmission}\DIFdelend \DIFaddbegin \DIFadd{In doing so, we reparameterize the model }\textit{\DIFadd{non-symptomatic}} \DIFadd{transmission, rather than the proportion of cases.
We then use $\delta_s$ to capture the decrease in transmission }\DIFaddend only after symptom \DIFdelbegin \DIFdel{onset}\DIFdelend \DIFaddbegin \DIFadd{onset---this means that $\delta_s$ is now independent of the amount of presymptomatic transmission}\DIFaddend .
...
Using the generalized non-symptomatic transmission model, we find a wide variety of scenarios for which peak fatalities occur at intermediate levels of non-symptomatic transmission in the presence of moderate to strong behavioral effects, \DIFdelbegin \DIFdel{$\delta > 0.6$ }\DIFdelend \DIFaddbegin \DIFadd{$\delta_s > 0.6$ }\DIFaddend (Supplementary Figure \DIFdelbegin \DIFdel{S1}\DIFdelend \DIFaddbegin \DIFadd{S3}\DIFaddend ; Table S2).
\DIFaddbegin \DIFadd{For example, when 40\% of non-symptomatic transmission is caused by pre-symptomatic transmission, a 60\% reduction in transmission after symptom onset is sufficient to drive the nonlinear effect of non-symptomatic transmission on epidemic fatality (peaking at around 10\% non-symptomatic transmission).
}\DIFaddend One exception is the extreme (and unrealistic) case, in which all non-symptomatic transmission is caused by pre-symptomatic transmission (i.e., there are no asymptomatic cases);
...
\DIFaddbegin \DIFadd{While 60\% reduction in transmission after symptom onset ($\delta_s = 0.6$) is still high, it is likely more feasible than reaching $\delta = 0.6$.
For example, \mbox{%DIFAUXCMD
\cite{kucharski2020effectiveness} }\hspace{0pt}%DIFAUXCMD
estimated that up to 64\% reduction in transmission would be possible under self-isolation and -quarantine as well as manual contact tracing of all contacts (and as low as 47\% reduction using digital contact tracing without manual tracing).
...
}\DIFaddend Hereafter, we focus on asymptomatic infections for simplicity, but our conclusions have implications for the more general case of non-symptomatic transmission.
\DIFaddbegin \DIFadd{We return to the discussion of values of $\delta$ later in the Discussion section.
}\DIFaddend 
...
\DIFaddbegin \DIFadd{The main conclusion of our analysis relies on having high values of $\delta$, the degree of transmission reduction among symptomatic individuals throughout the course of their infection.
...
We have argued for the feasibility of high $\delta_s$ values based on early estimates for the contact tracing effectiveness \mbox{%DIFAUXCMD
\citep{kucharski2020effectiveness}}\hspace{0pt}%DIFAUXCMD
, but it is uncertain what are the realistic ranges for $\delta_s$.
...
As the number of cases increase, $\delta$ will also increase, reflecting changes in awareness-driven behavior and intensity of non-pharmaceutical interventions \mbox{%DIFAUXCMD
\citep{weitz2020awareness}}\hspace{0pt}%DIFAUXCMD
.
...
\DIFaddbegin \DIFadd{Our conclusions echo earlier findings by \mbox{%DIFAUXCMD
\cite{gandon2001imperfect}}\hspace{0pt}%DIFAUXCMD
, who showed that vaccine-derived immunity against diseases can promote the evolution of more virulent strains in unvaccinated individuals.
...
\DIFaddbegin \DIFadd{Our work remains largely theoretical in illustrating the potential for asymptomatic infection and transmission to make population-level outcomes worse.
...
\DIFaddend As is increasingly evident, SARS-CoV-2 has proven hard to control in large part because transmission is often decoupled from symptoms. 
...
\dot{E} &= \beta_a S I_a + (1-\delta\DIFaddbegin \DIFadd{_s}\DIFaddend ) \beta_s S I_s - \nu E\\
\dot{I}_p &= \nu E - \sigma I_p\\
...
\begin{table}[h!]
  \begin{center}
    \begin{tabular}{c|l|c} % <-- Alignments: 1st column left, 2nd middle and 3rd right, with vertical lines in between
      ...
      $f$ & Fatality rate for symptomatic case & 0.01\\
      \hline
      \DIFdelbeginFL \DIFdelFL{$\delta$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$\delta_s$ }\DIFaddendFL & Reduction in symptomatic transmission rate & 0--1\\
    \end{tabular}
...
\begin{figure}[!ht]
\begin{center}
\DIFaddbeginFL \includegraphics[width=\textwidth]{figure_base_sens.ggp.pdf}
\caption{
\textbf{\DIFaddFL{Simulations of a model with asymptomatic transmission and symptom-responsive transmission reduction for a wide range of asymptomatic transmissibility.}}
\DIFaddFL{Total deaths as a function of the proportion of asymptomatic infections $p$ across a wide range scenarios for $\delta$.
...
\caption{
\textbf{\DIFaddFL{Simulations of a model with asymptomatic transmission and symptom-responsive transmission reduction for a wide range of asymptomatic transmissibility and a fixed $\mathcal{R}_0$ value at intermediate asymptomaticity.}}
\DIFaddFL{Total deaths as a function of the proportion of asymptomatic infections $p$ across a wide range scenarios for $\delta$.
...
\begin{figure}[!ht]
\begin{center}
\DIFaddendFL \includegraphics[width=0.8\textwidth]{diagram_sub.pdf}
\caption{
...
(B) Total deaths as a function of the proportion of non-symptomatic transmission $\phi$ across a wide range scenarios for \DIFdelbeginFL \DIFdelFL{$\delta$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$\delta_s$ }\DIFaddendFL and proportion of non-symptomatic transmission caused by the pre-symptomatic transmission, $\eta$ (between 0\% and 100\%). 
